...
void swapToNode(int id) {
    if (g_nodes[g_activeNode]) {
        ...
        g_nodes[g_activeNode]->beaconTracker = ::beaconTracker;
    ...
    ::beaconTracker = g_nodes[id]->beaconTracker;
    ::udp = g_nodes[id]->udp;
...
uint32_t micros() {
    return (uint32_t)(g_realTimeUs * g_drifts[g_activeNode] + g_offsets[g_activeNode]);
}
...
int main() {
    ...
    // Set Node 0 as the reference anchor manually for bootstrap
    swapToNode(0);
    localQuality = 1.0;
    localPhase = 0;
    localDrift = 0;

    printf("--- Phase 1: Convergence (300s) ---\n");
    run_sim(300);
    ...
    for (int i = 0; i < numNodes; i++) {
        ...
        printf("Node %d: Offset=%lld us, Quality=%.2f, Drift=%.6f\n",
               i, (long long)diff, localQuality, localDrift);
    }
