        ...
        if (currentTime - lastSendTime >= packetInterval) {
            ...
            if (activeNodes > 0) {
                ...
                if (avgPacketLoss > 0.1) {  // Too much loss
                    ...
                    packetInterval = packetInterval * 1.1;
                    if(packetInterval>MAX_PACKET_INTERVAL) {packetInterval=MAX_PACKET_INTERVAL;}  

                } else if (avgPacketLoss < 0.05) {  // Low loss
//                    packetInterval = max(packetInterval * 0.9, MIN_PACKET_INTERVAL);
                    packetInterval = packetInterval * 0.9;
                    if(packetInterval<MIN_PACKET_INTERVAL) {packetInterval=MIN_PACKET_INTERVAL;}  
