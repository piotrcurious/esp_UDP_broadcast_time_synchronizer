...
class Network:
    def __init__(self, loss_rate=0.01, latency_min=5, latency_max=15, jitter=1):
...
class KalmanPeerFilter:
    ...
    def update(self, measuredPhaseUs, measurementNoise):
        ...
        self.phaseUs += K0 * y; self.driftUsPerSec += K1 * y
...
class NTPNode:
    ...
    def step(self, real_time_ms, network):
        ...
        if real_time_ms - self.lastPollMs >= 1000:
            self.txSequence += 1
            pkt = {'type': 'POLL', 'src': self.node_id, 'seq': self.txSequence, 't1': nowUs, 'advPhase': self.localPhaseUs, 'advDrift': self.localDriftUsPerSec, 'advQuality': self.localQuality}
            network.broadcast(self.node_id, pkt, real_time_ms)
            self.pendingPolls[pkt['seq']] = nowUs
            self.lastPollMs = real_time_ms
        ...
        for r in self.pendingReplies:
            if nowUs >= r['sendAtUs']:
                reply = {'type': 'REPLY', 'src': self.node_id, 'dst': r['target'], 'echoSeq': r['poll']['seq'], 't1': r['poll']['t1'], 't2': r['rxUs'], 't3': nowUs, 'advPhase': self.localPhaseUs, 'advDrift': self.localDriftUsPerSec, 'advQuality': self.localQuality}
                network.send(self.node_id, r['target'], reply, real_time_ms)
            else: rem_replies.append(r)
...
def run_simulation(num_nodes=4, duration_s=600):
    ...
    for t_ms in range(0, duration_s * 1000):
        step_data = []
        for node in nodes: step_data.append(node.step(t_ms, network))
